...
# ---------- loaders ----------
def _load_age_index(path: str) -> Dict[str,int]:
    out={}
    try:
...
def _load_and_normalize_roster(path: str) -> List[Dict[str,Any]]:
    roster=[]
...
def _build_formation(self, formation: Dict[str,int], budget: int) -> Dict[str,Any]:
    ...
    def pick_balanced_role(role: str, needed_count: int):
        ...
        # Define thresholds for better quality
        top_tier_fm = 6.5
        mid_tier_fm = 6.0

        # If tiers are empty, fall back to percentile-based approach
        top_tier = [p for p in valid_pool if p.get("_fm", 0) >= top_tier_fm]
        mid_tier = [p for p in valid_pool if mid_tier_fm <= p.get("_fm", 0) < top_tier_fm]
        budget_tier = [p for p in valid_pool if p.get("_fm", 0) < mid_tier_fm]
...
def get_response(self, user_text: str, mode: str = "classic",
        ...
        budget_match = re.search(r"budget\s+(\d+)", lt)
        role_query = None
        budget = None
        ...
        # Detect role queries
        if any(x in lt for x in ["attaccanti", "attaccante", "forward", "punta"]):
            role_query = "A"
        elif any(x in lt for x in ["difensori", "difensore", "defender"]):
            role_query = "D"
        elif any(x in lt for x in ["centrocampisti", "centrocampista", "midfielder", "mezz"]):
            role_query = "C"
        elif any(x in lt for x in ["portieri", "portiere", "goalkeeper"]):
            role_query = "P"

        # Handle budget + role queries
        if budget and role_query:
            return self._handle_budget_role_query(budget, role_query, user_text)
...
class FantacalcioAssistant:
    def __init__(self) -> None:
        ...
        self.roster: List[Dict[str, Any]] = _load_and_normalize_roster(self.roster_json_path)
        self.external_youth_cache: List[Dict[str, Any]] = _load_external_youth_cache(self)

        # Initialize missing attributes
        self.season_filter = SEASON_FILTER or None
        self.age_index = _load_age_index(AGE_INDEX_PATH)
        self.override_roles = {}  # Initialize before loading overrides
        self.overrides = _load_overrides(AGE_OVERRIDES_PATH)
        ...
        _auto_detect_season(self)
        _apply_ages_to_roster(self)
        _make_filtered_roster(self)
    ...
    # ---------- Selettori e Gestori Query ----------

    def _extract_requested_k(self, text: str, default: int = 3) -> int:
        """Extract the number of players requested from the text."""
        match = re.search(r"(\d+)\s+(?:giocatori|nomi|profili)", text)
        if match:
            return max(1, int(match.group(1)))
        return default

    def _fallback_youth_from_cache(self, max_age: int, target_buckets: Optional[List[str]], k: int) -> List[Dict[str, Any]]:
        """Fetch youth players from external cache if available."""
        if not self.external_youth_cache:
            return []

        LOG.info(f"[Cache Fallback] Searching cache for U{max_age} (Buckets: {target_buckets}, K: {k})")
        results = []
        for p in self.external_youth_cache:
            if len(results) >= k:
                break

            # Role check
            if target_buckets and self._role_bucket(p.get("role") or "") not in target_buckets:
                continue
            ...
            if birth_year:
                ...
                # If no birth year, consider it invalid for precise age filtering
                LOG.debug(f"[Cache Fallback] Skipping player {p.get('name')} due to missing birth year.")

        LOG.info(f"[Cache Fallback] Found {len(results)} players in cache.")
        return results

    def _format_young_answer(self, items: List[Dict[str, Any]], prefix: str, age_limit: int) -> str:
        """Format the list of young players into a human-readable string."""
        if not items:
            return f"{prefix} non trovati."
        ...
        for i, p in enumerate(items):
            ...
            if age is not None and age <= age_limit:
                details.append(f"{age} anni")
            elif age is not None:
                details.append(f"{age} anni (limite U{age_limit})") # Indicate if slightly over
            else:
                details.append("Età N/D")

            if isinstance(fm, (int, float)):
                details.append(f"FM {fm:.2f}")
            if isinstance(price, (int, float)):
                details.append(f"€{int(round(price))}")
            else:
                details.append("Prezzo N/D")

            lines.append(f"- **{name}** ({team}) — " + ", ".join(details))

        return f"{prefix}:\n" + "\n".join(lines)

    def _suggest_young(self, max_age: int, target_buckets: Optional[List[str]], k: int) -> List[Dict[str, Any]]:
        """Suggest young players from the filtered roster."""
        LOG.info(f"[SuggestYoung] Finding U{max_age} (Buckets: {target_buckets}, K: {k})")
        pool = []
        for p in self.filtered_roster:
            if len(pool) >= k:
                break
            ...
            if birth_year:
                ...
                # If no birth year, try to guess from KM if it's a fallback scenario
                if max_age >= 22: # Only guess for U23 fallback
                    guessed_by = self._guess_birth_year_from_km(p.get("name", ""))
                    if guessed_by:
                        age = self._age_from_by(guessed_by)
                        if age is not None and age <= max_age:
                            # Cache the guessed age for future use
                            k_guess = _age_key(p.get("name",""), p.get("team",""))
                            if k_guess not in self.guessed_age_index:
                                self.guessed_age_index[k_guess] = guessed_by
                                p["birth_year"] = guessed_by # Update in place for this suggestion
                            pool.append(p)
                            LOG.debug(f"[SuggestYoung] Guessed age for {p.get('name')}: {age}")

        LOG.info(f"[SuggestYoung] Found {len(pool)} players.")
        # Sort by fantamedia, then price for consistency
        pool.sort(key=lambda x: (-(x.get("_fm") or 0.0), (x.get("_price") or 9999.0)))
        return pool

    def _handle_budget_role_query(self, budget: int, role: str, user_text: str) -> str:
        """Handle queries that specify both budget and role."""
        if role == "P":
            return self._handle_goalkeeper_request(user_text)
        elif role == "A":
            return self._answer_top_attackers_by_budget(budget)
        # Add other role-specific handlers here if needed
        else:
            # Generic budget + role handling (e.g., "top defenders budget 200")
            players = self._select_top_by_budget(role, budget, take=8)
            if players[0]: # Within budget
                lines = []
                for p in players[0]:
                    fm = p.get("_fm"); pr = p.get("_price"); vr = p.get("_value_ratio")
                    bits = []
                    ...
                    if isinstance(vr,(int,float)): bits.append(f"Q/P {(vr*100):.1f}%")
                    lines.append(f"- **{p.get('name','N/D')}** ({p.get('team','—')}) — " + ", ".join(bits))
                return f"🎯 **Top {role} entro {budget} crediti (ordine Q/P):**\n" + "\n".join(lines)
            elif players[1]: # FM only
                lines = []
                for p in players[1]:
                    fm = p.get("_fm")
                    bits = ["Prezzo N/D"]
                    if isinstance(fm,(int,float)): bits.insert(0, f"FM {fm:.2f}")
                    lines.append(f"- **{p.get('name','N/D')}** ({p.get('team','—')}) — " + ", ".join(bits))
                return f"ℹ️ **Top {role} per FM (prezzo mancante):**\n" + "\n".join(lines)
            else:
                return f"Non ho trovato giocatori {role} con budget {budget} crediti nel roster."
